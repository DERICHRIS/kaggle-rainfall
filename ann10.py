...
#cloud, sunshine, windspeed, maxtemp, pressure, mintemp
print(data.head())

X=data.drop(columns=['id','day','dewpoint','humidity','cloud','winddirection','rainfall'])
...
with mlflow.start_run():
    ...
    model.add(Dense(64, input_dim=X_train_scaled.shape[1], activation='sigmoid'))
    ...
    mlflow.log_param("model","ANN10")
    mlflow.log_metric("mean square error",mse)
    mlflow.log_metric("r2",r2)

    mlflow.sklearn.log_model(model,"ANN10")
